...
export default async function (m, modz = { user: {} }) {
  let _name = await modz.getName(m.sender)
  let sender = PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '')
  let chat = await modz.getName(m.chat)
  // let ansi = '\x1b['
  let img
  try {
    if (global.opts['img'])
  ...
  let me = PhoneNumber('+' + (modz.user?.jid).replace('@s.whatsapp.net', '')).getNumber('international')
  console.log(`
╭┈❲ ${chalk.redBright('%s')}
...
`.trim(),
 me + ' ~' + modz.user.name,
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (m.mentionedJid) for (let user of m.mentionedJid) log = log.replace('@' + user.split`@`[0], chalk.blueBright('@' + await modz.getName(user)))
    console.log(m.error != null ? chalk.red(log) : m.isCommand ? chalk.yellow(log) : log)
  }
  if (m.messageStubParameters) console.log(m.messageStubParameters.map(jid => {
    jid = modz.decodeJid(jid)
    let name = modz.getName(jid)
