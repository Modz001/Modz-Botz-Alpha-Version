...
async function start(file) {
    ...
    const modzFolder = join(dirname(currentFilePath), 'Modz')
    const totalFoldersAndFiles = await getTotalFoldersAndFiles(modzFolder)
    fs.readdir(modzFolder, async (err, files) => {
        if (err) {
...
• Creator Script: ${chalk.bold.cyan('Zidan')}`)
